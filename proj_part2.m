...
% The first code we got working was BCH with BPSK modulation. Although we
% got this encoding working, we decided to move on to convolutional
% encoding and so we never finalized this into a final draft.
%
% As such the code below is not cleaned and efficient but rather 
% extensively documented. This was done by our group so that we all 
% understand the what, how and why behind every line.
...
% CODEWORD LENGTH:
j = 2^X-1; % default is 15, max allowed is 65,535
...
for i = 1:numIter
    ...
    for j = 1:lenSNR % one iteration of the simulation at each SNR Value
        ...
        % Changed noise model        
        txNoisy_rs = awgn(txChan_rs, 3+SNR_Vec(j), 'measured'); % Add AWGN
        txNoisy_noconv = awgn(txChan_noconv, 3+SNR_Vec(j), 'measured'); % no coding
        
        rx_demod_signal = equalize(eqobj, txNoisy_rs, train_seq);
        yd_noconv = equalize(eqobj, txNoisy_noconv, trainseq_noconv);
        
        % de-modulation
        if isequal(modulation, 1)
            rx_rs = pamdemod(rx_demod_signal, M);  % PAM
        elseif isequal(modulation, 2)
            rx_rs = qamdemod(rx_demod_signal, M);  % QAM
            rx_noconv = qamdemod(yd_noconv,M);
        else
            rx_rs = pskdemod(rx_demod_signal, M);  % PSK
        end
        
        rxbits = msg2bits(rx_rs,M);
...
BER_Rate = [ber, ber_noconv, berTheory]';
